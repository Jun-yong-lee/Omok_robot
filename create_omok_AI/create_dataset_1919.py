...
# 데이터 전처리 과정
game_rule = 'Freestyle_'  # 프리스타일(3x3가능 5개 이상만 돌을 연속으로 두면 승리)
base_path = '/PycharmProjects/Omok_Robot/gomocup_Freestyle_results'  # 전처리전 데이터가 저장되어 있는 경로

output_path = os.path.join('dataset_test_1919', os.path.basename(base_path))  # dataset\gomocup2021results\
...
for index, file_path in enumerate(tqdm(file_list)):
    ...
    w, h = int(w) - 1, int(h) - 1  # 문자에서 인트형으로 형 변환
